							...
							const char	*helper_executable/*=NULL*/)
{
...
#if PATCHED
	LPCSTR M_NAME = m_name;

	LPCSTR CNP_FAIL_MSG = "CreateNamedPipe failed";
	LPCSTR CL_FAIL_MSG = "ConsoleLogger failed";
	LPCSTR FIND_HELPER_FAIL_MSG = "Helper executable not found";
	LPCSTR WF1_FAIL_MSG = "WriteFile failed(1)";
	LPCSTR WF2_FAIL_MSG = "WriteFile failed(2)";
	LPCSTR WF3_FAIL_MSG = "WriteFile failed(3)";
#else
	auto& M_NAME = m_name;

	const char* CNP_FAIL_MSG = "CreateNamedPipe failed";
	const char* CL_FAIL_MSG = "ConsoleLogger failed";
	const char* WStr FIND_HELPER_FAIL_MSG = "Helper executable not found";

	const char* WF1_FAIL_MSG = "WriteFile failed(1)";
	const char* WF2_FAIL_MSG = "WriteFile failed(2)";
	const char* WF3_FAIL_MSG = "WriteFile failed(3)";
#endif
	...
	if (m_hPipe == INVALID_HANDLE_VALUE)
	{	// failure
		MessageBox(NULL, CNP_FAIL_MSG, CL_FAIL_MSG, MB_OK);
		return -1;
	}
...
#if PATCHED
	LPSTR CMD_LINE = cmdline;
#else
	auto& CMD_LINE = cmdline;
#endif

	sprintf(cmdline, "%s %s", helper_executable, logger_name);
	BOOL bRet = CreateProcess(NULL, CMD_LINE, NULL, NULL, FALSE, CREATE_NEW_CONSOLE, NULL, NULL, &si, &pi);
	if (!bRet)
	{	// on failure - try to get the path from the environment
		...
		if (path)
		{
			...
			bRet = CreateProcess(NULL, CMD_LINE, NULL, NULL, FALSE, CREATE_NEW_CONSOLE, NULL, NULL, &si, &pi);
		}
		if (!bRet)
		{
			MessageBox(NULL, FIND_HELPER_FAIL_MSG, CL_FAIL_MSG, MB_OK);
			CloseHandle(m_hPipe);
			m_hPipe = INVALID_HANDLE_VALUE;
			return -1;
		}
	...
	{
		MessageBox(NULL, WF1_FAIL_MSG, CL_FAIL_MSG, MB_OK);
		DisconnectNamedPipe(m_hPipe);
		CloseHandle(m_hPipe);
		m_hPipe=INVALID_HANDLE_VALUE;
		return -1;
	...
	{	// Send buffer-size
		...
		if (cbWritten!=strlen(buffer))
		{
			MessageBox(NULL, WF2_FAIL_MSG, CL_FAIL_MSG, MB_OK);
			DisconnectNamedPipe(m_hPipe);
			CloseHandle(m_hPipe);
			m_hPipe=INVALID_HANDLE_VALUE;
			return -1;
	...
	if (cbWritten!=1)
	{
		MessageBox(NULL, WF3_FAIL_MSG, CL_FAIL_MSG, MB_OK);
		DisconnectNamedPipe(m_hPipe);
		CloseHandle(m_hPipe);
		m_hPipe=INVALID_HANDLE_VALUE;
		return -1;
